...
Page({
  ...
  getNewSong: function () {
    API.getNewSong({}).then(res => {
      if (res.code === 200) {
        ...
        for (let i = 0; i <= this.data.newsong.length - 1;i++){ //循环打印出其id
          waitForPlay.push(this.data.newsong[i].id) //循环push ID 到waitForPlay数组
  ...
  changepage : function(e){
    let offsetLeft = e.currentTarget.offsetLeft; //获取swiper的current值
